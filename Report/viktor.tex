...
current date of the year, regardless of data set. This remove cases 
where data points might be missing. As a final requirement, the month 
can at most represent July. At the final iteration the first registered 
date and temperature is stored. Note the method does not divide the 
year into 52 weeks but instead look for the first 7 succeeding 
iterations which satisfy the definition we use.
\begin{lstlisting}
